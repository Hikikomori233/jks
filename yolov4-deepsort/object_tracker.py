...
flags.DEFINE_string('video', './data/video/example_02.mp4', 'path to input video or set to 0 for webcam')
flags.DEFINE_string('output', './outputs/test3.avi', 'path to output video')
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])),
                          (int(bbox[0]) + (len(class_name) + len(str(track.track_id))) * 17, int(bbox[1] + 20)), color,
                          -1)
            ...
            if not "%d" % track.track_id in object_dic:
                ...
                object_dic["%d" % track.track_id] = {"trace": [], 'traced_frames': 10, "tested": 0}
            ...
            # 如果有，直接写入
                ...
                object_dic["%d" % track.track_id]["trace"].append(center)
                object_dic["%d" % track.track_id]["traced_frames"] += 1
                object_dic["%d" % track.track_id]["tested"] = False
            ...
            if FLAGS.roi:
                ...
                #pts1 = np.array([[173, 456], [574, 515]], np.int32)
                #pts1 = pts1.reshape((-1, 1, 2))
                #pts1 = np.array([0, height//2], [width, height//2])
               ...
               # ycd = round(-0.59 * x + 853.710, 2)
                ...
                if y > height//2 and track.tested==False:
                    target.append(x)
                    total_target.append(x)
                    track.tested = True
                    cv2.putText(frame, str('enter'), (int(bbox[2] - 65), int(bbox[3] - 5)),
                                cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 255, 0), 1)
        cv2.putText(frame, "ROI count: {}".format(str(len(total_target))),
         (100, 50), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2,(255, 0, 0), 2)
        ...
        for s in object_dic:
            ...
            if len(object_dic["%d" % i]["trace"]) > 2:
                y_s = object_dic["%d" % i]["trace"][1][1]
                y_end = object_dic["%d" % i]["trace"][-2][1]
                ...
                direction = y_end - y_s
                if direction > 0 and (y_end > height // 2 )and (y_s <= height // 2 ):
                    if i not in total_up:
                        total_up.append(i)
                elif direction<0 and (y_end <= height // 2 )and (y_s > height // 2 ):
                    if i not in total_down:
                        total_down.append(i)
        cv2.putText(frame, "Up count: {}".format(str(len(total_up))),
                (50, 100), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 0, 0), 2)
        cv2.putText(frame, "Up count: {}".format(str(len(total_down))),
                (50, 200), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 0, 0), 2)
        # 对已经消失的目标予以排除
        for s in object_dic:
